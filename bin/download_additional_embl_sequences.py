      ...
      with open("ipd-mhc-mane-" + datetime.today().strftime('%Y-%m-%d') + "_" + str(embl_accession) + ".gbk", "a") as mane:
        ...
        if not ipd_embl.startswith('ID'):
          ...
          for record in SeqIO.parse("response.embl", "embl"):
              ...
              # if rhesus sequence
              if record.annotations['organism'].startswith('Macaca mulatta'):
                SeqIO.write(record, mamu, "genbank")
  
              # if cyno sequence
              if record.annotations['organism'].startswith('Macaca fascicularis'):
                SeqIO.write(record, mafa, "genbank")
  
              # if mane sequence
              if record.annotations['organism'].startswith('Macaca nemestrina'):
