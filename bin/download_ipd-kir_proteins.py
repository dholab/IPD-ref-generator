...
with open("ipd-kir-nhp-prot-" + datetime.today().strftime('%Y-%m-%d') + "_" + str(ipd_number) + ".fasta", "a") as all_nhp:

  # create output genbank file for rhesus only
  with open("ipd-kir-mamu-prot-" + datetime.today().strftime('%Y-%m-%d') + "_" + str(ipd_number) + ".fasta", "a") as mamu:

    # create output genbank file for cyno only
    with open("ipd-kir-mafa-prot-" + datetime.today().strftime('%Y-%m-%d') + "_" + str(ipd_number) + ".fasta", "a") as mafa:

      # create output genbank file for mane only
      with open("ipd-kir-mane-prot-" + datetime.today().strftime('%Y-%m-%d') + "_" + str(ipd_number) + ".fasta", "a") as mane:
        ...
        # get record
        u = requests.get("https://www.ebi.ac.uk/Tools/dbfetch/dbfetch?db=ipdnhkirpro;id=" + nhp_id + ";style=raw")
