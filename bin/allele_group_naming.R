...
# setwd(filepath)
fasta <- read.delim("ipd-mhc-mamu-2022-07-11_cleaned.miseq.trimmed.deduplicated.sorted.fasta", header = F)
# fasta <- read.delim(args[1], header = F)

file_basename <- "ipd-mhc-mamu-2022-07-11_cleaned.miseq.trimmed.deduplicated.sorted"
# file_basename <- str_remove(args[1], ".fasta")

if (TRUE %in% grepl(">Mamu-", fasta[,1])){
  animal <- ">Mamu-"
