...
def collate_alleles(old_database: Optional[str], gene: str, current_date: str) -> Optional[int]:
    ...
    with ExitStack() as stack:
        all_nhp = stack.enter_context(open(f"ipd-{gene_str}-prot-nhp-{current_date}.fasta","a", encoding="utf-8"))
        mamu = stack.enter_context(open(f"ipd-{gene_str}-prot-mamu-{current_date}.fasta", "a", encoding="utf-8"))
        mafa = stack.enter_context(open(f"ipd-{gene_str}-prot-mafa-{current_date}.fasta", "a", encoding="utf-8"))
        mane = stack.enter_context(open(f"ipd-{gene_str}-prot-mane-{current_date}.fasta", "a", encoding="utf-8"))
