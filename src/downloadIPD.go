...
func defineUrls(maxCount int, gene string) []string {
	...
	if gene == "MHC" {
		...
		result := make([]string, maxCount)
		for i := 1; i <= maxCount; i++ {
			hlaID := fmt.Sprintf("NHP%05d", i)
			result[i-1] = "https://www.ebi.ac.uk/Tools/dbfetch/dbfetch?db=imgthla;id=" + hlaID + "&style=raw"
		}
		return result
		...
		for i := 1; i <= maxCount; i++ {
			nhpID := fmt.Sprintf("NHP%05d", i)
			result[i-1] = "https://www.ebi.ac.uk/Tools/dbfetch/dbfetch?db=ipdmhcpro;id=" + nhpID + "&style=raw"
		}
		return result
		...
		for i := 1; i <= maxCount; i++ {
			nhpID := fmt.Sprintf("NHP%05d", i)
			result[i-1] = "https://www.ebi.ac.uk/Tools/dbfetch/dbfetch?db=ipdnhkirpro;id=" + nhpID + "&style=raw"
		}
		return result
	}

	// Return an empty slice if gene is none of the above
	return []string{}
